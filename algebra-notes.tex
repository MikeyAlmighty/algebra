...
\begin{tikzpicture}[scale=0.7]
  ...
  \draw[domain=-1.2:1.2,smooth,variable=\x,blue,thick]
    plot ({\x},{5*\x});
  \node[blue] at (0.8,3.2) {\scriptsize \( y = 5x \)};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[scale=0.8]
  \draw[very thin, color=gray!30] (-5,-5) grid (5,5);
  \draw[->,thick] (-5.2,0) -- (5.2,0) node[right] {\( x \)};
  \draw[->,thick] (0,-5.2) -- (0,5.2) node[above] {\( y \)};
  \draw[domain=0.5:5, smooth, variable=\x, blue, thick] plot ({\x}, {6/\x});
  \draw[domain=-5:-0.5, smooth, variable=\x, blue, thick] plot ({\x}, {6/\x});
  \node[blue] at (3,1.4) {\scriptsize \( y = \frac{6}{x} \)};
\end{tikzpicture}
\end{center}
...
\textbf{Answer:} The wall is \fbox{24 meters} away from the starting place.

\subsection{Systems with Non-linear Equations}

A \textbf{system with non-linear equations} occurs when at least one of the equations is not linear (e.g., involves squares, square roots, products of variables, etc.).
...
So:
\[
x = 0 \quad \text{or} \quad x = \frac{12}{5}
\]
...
\textbf{Case 2:} \( x = \frac{12}{5} \)
From \( y = 2x - 3 \):
\[
y = \frac{24}{5} - 3 = \frac{9}{5}
\]
...
\textbf{Final answer:}
\[
\boxed{(0, -3) \quad \text{and} \quad \left(\frac{12}{5}, \frac{9}{5}\right)}
\]
...
\textbf{Step 1: Solve the linear equation for \(y\).}
From \(2x + y = 11\):
\[
y = 11 - 2x
\]

\textbf{Step 2: Substitute into the quadratic equation.}
\[
2x^2 - 12x - (11 - 2x) + 19 = 0
\]
\[
...
\textbf{Step 3: Simplify and solve for \(x\).}
Divide through by 2:
\[
x^2 - 5x + 4 = 0
\]
...
\textbf{Step 4: Find the corresponding \(y\) values.}
If \(x = 4\):
\[
y = 11 - 2(4) = 3
\]
...
\[
\boxed{(4, 3) \quad \text{and} \quad (1, 9)}
\]
